import { XCircle, ShieldCheck, ShieldAlert, FileText, KeyRound } from 'lucide-react';

export default function ClarityPopup({ riskLevel, title, message, details, onClose }) {
  const isDanger = riskLevel === 'danger';
  const bgColor = isDanger ? 'bg-slate-800/80 border-red-500' : 'bg-slate-800/80 border-green-500';
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 backdrop-blur-md">
      ...
      <div className={`relative w-full max-w-2xl rounded-2xl border ${bgColor} p-8 text-white shadow-2xl shadow-black/40 animate-fade-in-scale`}>
        {/* Close Button */}
                 ...
                 <div className="space-y-4 rounded-lg bg-black/20 p-4 border border-slate-700">
                    <div className="flex items-center gap-4">
                     ...
                     <div className="flex items-center gap-4">
                        ...
                        <div className="flex-grow">
                            <h3 className="font-semibold text-slate-200">Permissions Granted</h3>
                            <p className="font-mono text-sm text-amber-400">{details.permissions}</p>
                        </div>
                    </div>
                 </div>
            </div>
        ...
        <div className="mt-8 flex justify-end gap-4">
          ...
          {!isDanger && <button className="px-6 py-2 rounded-lg font-bold bg-green-600 hover:bg-green-700 transition-all">Approve</button>}
        </div>
      </div>
    </div>
  );
