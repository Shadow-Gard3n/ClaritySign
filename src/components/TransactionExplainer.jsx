  ...
  return (
    <div className="group relative bg-cyber-surface backdrop-blur-md p-8 rounded-lg shadow-2xl border border-cyber-cyan/20 transition-all duration-300 hover:border-cyber-cyan/50 hover:-translate-y-2 hover:shadow-glow-cyan">
      ...
      <div className="flex items-center mb-6">
        <h2 className="text-2xl font-bold text-cyber-cyan">Transaction Explainer</h2>
      </div>

      <textarea
        className="w-full h-48 bg-cyber-bg/70 text-cyber-blue p-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyber-cyan transition-all placeholder-cyber-blue/50"
      ...
      <button
        ...
        disabled={isLoading}
        className="mt-6 w-full flex items-center justify-center bg-cyber-cyan/80 hover:bg-cyber-cyan text-cyber-bg font-bold py-3 px-4 rounded-lg transition-all duration-300 transform hover:scale-105"
      >
        {isLoading ? (
      ...
      {!isLoading && (explanation || risk) && (
        <div className="mt-8 space-y-4 animate-fade-in-up">
          {explanation && (
            <div className="p-4 bg-cyber-surface rounded-lg">
              <h3 className="font-semibold text-lg text-cyber-cyan">Summary:</h3>
